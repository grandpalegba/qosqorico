...
function getSeedReviews(providerId) {
  ...
  return Array.from({ length: 4 }, (_, i) => ({
    ...
    comment_narrative: narrativeComments[seededNum(providerId + i + "nc", 0, narrativeComments.length)],
    comment_merchant: merchantComments[seededNum(providerId + i + "mc", 0, merchantComments.length)],
    ...
    created_date: new Date(Date.now() - seededNum(providerId + i + "date", 0, 30) * 86400000).toISOString()
  }));
}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        ...
        <ResponsiveContainer width="100%" height={170}>
          <RadarChart data={criteria.map(c => ({
            ...
            <Radar dataKey="value" stroke={criteria[0].color} fill={criteria[0].color} fillOpacity={0.2} strokeWidth={2} />
          </RadarChart>
        </ResponsiveContainer>
          ...
          {criteria.map(c => (
            <div key={c.key}>
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="text-[11px] font-bold" style={{ color: c.color }}>{scores[c.key]?.avg}/100</span>
              </div>
              <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                <div className="h-full rounded-full transition-all duration-700" style={{ width: `${scores[c.key]?.avg}%`, backgroundColor: c.color }} />
              </div>
            </div>
          ))}
        </div>
...
export default function ProviderDashboard({ provider }) {
  ...
  async function loadReviews() {
    setLoading(true);
    const real = await base44.entities.Review.filter({ provider_id: provider.id }, "-created_date", 20);
  ...
  function buildScores(criteria) {
    return Object.fromEntries(criteria.map(c => {
      ...
      const avg = vals.length ? Math.round(vals.reduce((a, b) => a + b, 0) / vals.length) : seededNum(provider.id + c.key, 62, 97);
                  ...
                  <div className="flex gap-1 flex-wrap justify-end">
                    {MERCHANT_CRITERIA.map(c => (
                      <span key={c.key} className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-muted whitespace-nowrap" style={{ color: c.color }}>
                        {(lang === "es" ? c.labelEs : c.labelEn).slice(0, 3)} {r[c.key]}
                      </span>
                    ))}
                  </div>
                </div>
